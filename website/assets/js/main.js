...
function initLoginForm() {
  ...
  loginForm.addEventListener('submit', (e) => {
    ...
    if (!studentIdInput || !studentIdInput.value.trim()) {
      if (studentIdInput) studentIdInput.classList.add('is-invalid');
      isValid = false;
    }
    ...
    if (isValid) {
      ...
      if (loginAlert) {
        ...
        loginAlert.innerHTML = `
          ...
          <div><strong>Notice:</strong> Authenticating demo session... Redirecting to portal dashboard.</div>
        `;
...
function initEventButtons() {
  ...
  regButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.preventDefault();
      const eventTitle = btn.getAttribute('data-event-title') || 'this event';
      alert(`Registration for "${eventTitle}" requires student verification. Please log into the Student Portal.`);
    });
  });
}
